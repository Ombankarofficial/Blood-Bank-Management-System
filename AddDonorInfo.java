...
public class AddDonorInfo extends JFrame implements ActionListener{
	...
	private PreparedStatement ps,pselect;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddDonorInfo frame = new AddDonorInfo();
	...
	public void createGui()
	{
		...
		txtphn.setFont(new Font("Tahoma", Font.PLAIN, 10));
	...
	public void actionPerformed(ActionEvent e) {
...

		{
			...
			try {
				...
				String strinsert = "insert into  donorinfo values(?,?,?,?,?,?,?,?)";
				...
				if (row > 0) {
					...
					pselect.setString(1, bloodgroup);
					//System.out.println(ps);
					rs=pselect.executeQuery();
				if(rs.next()==true)
					
				{
						int bags=rs.getInt("noofbags");
				}
						//txtp.setText(bags));
				
					this.dispose();
					Donation d=new Donation(phoneno,bb,type);
					d.setVisible(true);

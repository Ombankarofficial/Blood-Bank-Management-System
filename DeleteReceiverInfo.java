...
public class DeleteReceiverInfo extends JFrame implements ActionListener{
	...
	public void fillcombo()
	 {
		 try
		 {
			 String strsql="select phoneno from receiverinfo";
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DeleteReceiverInfo frame = new DeleteReceiverInfo();
	 ...
	 * Create the frame.
	 */
	public DeleteReceiverInfo() {
		con=CrudOperation.createConnection();
		setExtendedState(Frame.MAXIMIZED_BOTH);
		setTitle("DELETE RECEIVER");
		setIconImage(Toolkit.getDefaultToolkit().getImage(DeleteReceiverInfo.class.getResource("/bb/images/bb4.jpg")));
		createGui();
	}
...
try {
				...
				String strsql="select * from receiverinfo where phoneno=?";
	...
	int option=JOptionPane.showConfirmDialog(this,"ARE YOU SURE YOU WANT TO DELETE THIS RECEIVER");
			//System.out.println(option);
			if(option==0)
			{
				try {
					String strdelete="delete from receiverinfo where phoneno=?";
					psdelete=con.prepareStatement(strdelete);
					psdelete.setString(1,phone);
					int row=psdelete.executeUpdate();
					if(row>0)
						JOptionPane.showMessageDialog(this, "RECEIVER DELETED");
